...
var locationBroker = require('../../../websocket/locationBroker');
...
function validateRequestingDeviceKeyForGetLocation(requestConfig, callback) {
    requesterAuth.validateRequestingDeviceKeyForGetLocation(requestConfig, callback);
}

function validateDeviceKeyWithRequirement(deviceId, providedDeviceKey, requireConfiguredKey, callback) {
    deviceKeyUtils.getDeviceKey(deviceId, function(error, storedKey) {
        if (error) {
            return callback(error);
        }

        var hasConfiguredKey = storedKey !== null && storedKey !== undefined;

        if (requireConfiguredKey && !hasConfiguredKey) {
            return callback(new errors.ForbiddenError('DeviceKey must be set for this device'));
        }
        ...
        if (!providedDeviceKey || providedDeviceKey === null || providedDeviceKey === '') {
            return callback(new errors.ForbiddenError('DeviceKey is required for this device'));
        }

        deviceKeyUtils.validateDeviceKey(deviceId, providedDeviceKey, function(validationError, isValid) {
            if (validationError) {
                return callback(validationError);
            }

            if (!isValid) {
                return callback(new errors.ForbiddenError('DeviceKey does not match'));
            }
...
function updateLocation(req, res, next) {
    ...
    var locationsToPublish = locations.map(function(loc) {
        return loc.data();
    });
    ...
    chain
        .seq(function() {
            locationBroker.publishLocations(locationsToPublish, this);
        })
        .seq(function() {
            var eyes = ['x', '!', '^', '°'];
...
function getLocation(req, res, next) {
    ...
    var requestConfig = req.MIATARU.config;
    var requestingDeviceId = requestConfig.requestMiataruDeviceID();
    var responseLocations = new Array(req.MIATARU.devices.length);
    ...
    req.MIATARU.devices.forEach(function(device, index) {
        chain.par(function() {
            var done = this;
            ...
            db.get(kb.build(targetDeviceId, KEY_LAST), function (error1, reply) {
                ...
                allowedDevicesUtils.checkAccess(
                    ...
                    function(error2, hasAccess) {
                        ...
                        if (!hasAccess) {
                            responseLocations[index] = null;
                            return done();
                        }

                        // Access granted, return location data (including optional slogan)
                        if (reply != null) {
                            var locationData;
                            try {
                                locationData = JSON.parse(reply);
                            } catch (parseError) {
                                return done(new errors.InternalServerError('Invalid location data format'));
                            }

                            db.get(kb.build(targetDeviceId, KEY_SLOGAN), function(error3, sloganReply) {
                                if (error3) {
                                    return done(error3);
                                }
                                ...
                                responseLocations[index] = locationData;
                                done();
                            });
                        } else {
                            responseLocations[index] = null;
    ...
    chain.seq(function() {
        res.send((new models.ResponseLocation(responseLocations)).data());
    })
    .catch(next);
}
...
/**
 * SetDeviceSlogan
 * Sets an optional slogan string (max 40 chars) for a device.
 * Requires that the device has a configured DeviceKey and a valid matching key in the request.
 * @param req
 * @param res
 * @param next
 */
function setDeviceSlogan(req, res, next) {
    var request = req.MIATARU.request;
    var deviceId = request.deviceId();
    var deviceKey = request.deviceKey();
    var slogan = request.slogan();
    var sloganKey = kb.build(deviceId, KEY_SLOGAN);

    seq()
        .seq(function() {
            var done = this;
            validateDeviceKeyWithRequirement(deviceId, deviceKey, true, done);
        })
        .seq(function() {
            var done = this;
            db.set(sloganKey, slogan, function(error) {
                if (error) {
                    return done(error);
                }
        ...
        .seq(function() {
            res.send((new models.ResponseSetDeviceSlogan()).data());
        })
        .catch(function(error) {
            if (error instanceof errors.ForbiddenError) {
                return next(error);
...
/**
 * GetDeviceSlogan
 * Reads an optional slogan string for a target device.
 * Requesting device must be authenticated with RequestDeviceID + RequestDeviceKey.
 * Access is independent from allowed devices list and each successful read on an existing slogan
 * is tracked in the target device's visitor history.
 * @param req
 * @param res
 * @param next
 */
function getDeviceSlogan(req, res, next) {
    var request = req.MIATARU.request;
    var targetDeviceId = request.deviceId();
    var requestDeviceId = request.requestDeviceID();
    var requestDeviceKey = request.requestDeviceKey();
    var sloganKey = kb.build(targetDeviceId, KEY_SLOGAN);

    seq()
        .seq(function() {
            var done = this;
            validateDeviceKeyWithRequirement(requestDeviceId, requestDeviceKey, true, done);
        })
        .seq(function() {
            var done = this;
            db.get(sloganKey, function(error, reply) {
                if (error) {
                    return done(error);
                }

                done.vars = done.vars || {};
                done.vars.slogan = reply;
                done();
            });
        })
        .seq(function() {
            var slogan = this.vars.slogan;

            // Track visitor only if target has a slogan entry.
            if (slogan !== null && slogan !== undefined) {
                var visitKey = kb.build(targetDeviceId, KEY_VISIT);
                var visitorObject = request.requestVisitorObject(targetDeviceId);
                if (visitorObject !== null) {
                    recordVisitorHistory(visitKey, JSON.stringify(visitorObject));
                }
            }

            res.send((new models.ResponseDeviceSlogan(
                targetDeviceId,
 ...
 * Returns if a target device has a configured DeviceKey and if allowed devices list is enabled.
 * Requesting device must be authenticated with RequestDeviceID + RequestDeviceKey.
 ...
 * @param next
 */
function getDeviceSecurityStatus(req, res, next) {
    var request = req.MIATARU.request;
    var targetDeviceId = request.deviceId();
    var requestDeviceId = request.requestDeviceID();
    var requestDeviceKey = request.requestDeviceKey();
    var targetLastKey = kb.build(targetDeviceId, KEY_LAST);
    ...
    seq()
        ...
        .par('targetHasLastLocation', function() {
            db.exists(targetLastKey, this);
        })
        .par('targetStoredKey', function() {
            deviceKeyUtils.getDeviceKey(targetDeviceId, this);
        })
        .par('allowedDeviceListEnabled', function() {
            allowedDevicesUtils.isAllowedDeviceEnabled(targetDeviceId, this);
        })
        .seq(function() {
            var vars = this.vars || {};
            var hasDeviceKey = vars.targetStoredKey !== null && vars.targetStoredKey !== undefined;
            var isAllowedDeviceListEnabled = vars.allowedDeviceListEnabled === true;
            var hasLastLocation = Number(vars.targetHasLastLocation) === 1;
            ...
            res.send((new models.ResponseDeviceSecurityStatus(
                targetDeviceId,
                hasDeviceKey,
 ...
 * Returns the complete allowed device list for a device.
 * Device owners authenticate with DeviceID + DeviceKey using the same semantics as setAllowedDeviceList.
 * @param req
 * @param res
 * @param next
 */
function getAllowedDeviceList(req, res, next) {
    var request = req.MIATARU.request;
    var targetDeviceId = request.deviceId();
    var deviceKey = request.deviceKey();
    ...
    seq()
        .seq(function() {
            var done = this;
            deviceKeyUtils.validateDeviceKey(targetDeviceId, deviceKey, function(error, isValid) {
                if (error) {
                    return done(error);
                }
        ...
        .par('allowedDevices', function() {
            allowedDevicesUtils.getAllowedDevices(targetDeviceId, this);
        })
        .seq(function() {
            var vars = this.vars || {};

            res.send((new models.ResponseAllowedDeviceList(
                targetDeviceId,
                vars.allowedDeviceListEnabled === true,
                vars.allowedDevices || []
            )).data());
        })
        .catch(function(error) {
...
module.exports = {
    ...
    setDeviceSlogan: setDeviceSlogan,
    getDeviceSlogan: getDeviceSlogan,
    getDeviceSecurityStatus: getDeviceSecurityStatus,
    getAllowedDeviceList: getAllowedDeviceList
};
