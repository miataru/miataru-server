...
function getLocationHistory(req, res, next) {
    ...
    seq()
        ...
        .par('list', function() {
            db.lrange(key, locationRequest.amount() * -1, -1, this);
        })
        .seq(function() {
            res.send((new models.ResponseLocationHistory(this.vars.listLength, this.vars.list)).data());
        })
...
function updateLocation(req, res, next) {
    ...
    if(requestConfig.enableLocationHistory()) {
        locations.forEach(function(loc, idx) {
            ...
            chain
                ...
                .seq(function() {
                    db.ltrim(historyKey, 0, configuration.maximumNumberOfHistoryItems, this);
                });
    ...
    chain
        .seq(function() {
            var eyes = ['x', '!', '^', '°'];
            res.send((new models.ResponseFish(eyes[parseInt(Math.random() * eyes.length, 10)])).data());
        })
        .catch(next);
}
