...
RequestLocation.prototype._validate = function() {
    ...
    validation.validateNumericField(this._timestamp, 'Timestamp', { required: true, integer: true, min: 1 });
    validation.validateNumericField(this._longitude, 'Longitude', { required: true, min: -180, max: 180 });
    validation.validateNumericField(this._latitude, 'Latitude', { required: true, min: -90, max: 90 });
    validation.validateNumericField(this._horizontalAccuracy, 'HorizontalAccuracy', { required: true, min: 0, max: 10000 });

    if (this._speed !== -1) {
        validation.validateNumericField(this._speed, 'Speed', { min: 0, max: 1000 });
    }

    if (this._batteryLevel !== -1) {
        validation.validateNumericField(this._batteryLevel, 'BatteryLevel', { min: 0, max: 100 });
    }

    if (this._altitude !== -1) {
        validation.validateNumericField(this._altitude, 'Altitude', { min: -1000, max: 100000 });
    }
};
