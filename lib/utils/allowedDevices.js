    ...
    isAllowedDeviceEnabled(deviceId, function(error, isEnabled) {
        ...
        db.hgetall(hashKey, function(error, hashData) {
            ...
            Object.keys(hashData).forEach(function(key) {
                var parsedField = parseAllowedDeviceField(key);

                if (!parsedField) {
                    return;
                }

                var allowedDeviceId = parsedField.deviceId;
                var accessType = parsedField.accessType;
...
function parseAllowedDeviceField(fieldKey) {
    var currentLocationSuffix = ':' + ACCESS_TYPE_CURRENT_LOCATION;
    var historySuffix = ':' + ACCESS_TYPE_HISTORY;

    if (typeof fieldKey !== 'string') {
        return null;
    }

    if (fieldKey.slice(-currentLocationSuffix.length) === currentLocationSuffix) {
        return {
            deviceId: fieldKey.slice(0, -currentLocationSuffix.length),
            accessType: ACCESS_TYPE_CURRENT_LOCATION
        };
    }

    if (fieldKey.slice(-historySuffix.length) === historySuffix) {
        return {
            deviceId: fieldKey.slice(0, -historySuffix.length),
            accessType: ACCESS_TYPE_HISTORY
        };
    }
